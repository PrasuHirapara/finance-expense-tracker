...
class InvestmentEntryFormPage extends StatefulWidget {
  ...
  @override
  State<InvestmentEntryFormPage> createState() =>
      _InvestmentEntryFormPageState();
}
        ...
        appBar: AppBar(
          title: BlocBuilder<InvestmentFormBloc, InvestmentFormState>(
            builder: (context, state) {
              final isEdit = state.existingId != null;
              return Text(isEdit ? 'Edit Investment' : 'Add Investment');
            },
          ),
        ),
        body: BlocBuilder<InvestmentFormBloc, InvestmentFormState>(
          builder: (context, state) {
            if (state.status == InvestmentFormStatus.loading ||
                state.status == InvestmentFormStatus.initial) {
                ...
                children: <Widget>[
                  ...
                  AppSelectField<int>(
                    ...
                    onChanged: (value) {
                      context.read<InvestmentFormBloc>().add(
                        InvestmentFormCategoryChanged(value),
                      );
                    },
                  ),
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      errorText:
                          state.showValidation && state.symbol.trim().isEmpty
                          ? 'Enter symbol name'
                          : null,
                    ),
                    onChanged: (value) {
                      context.read<InvestmentFormBloc>().add(
                        InvestmentFormSymbolChanged(value),
                      );
                    },
                  ),
                  const SizedBox(height: 16),
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      errorText: state.showValidation
                          ? (state.qty == null || state.qty! <= 0
                                ? 'Enter valid quantity'
                                : (state.qty! < state.soldQty
                                      ? 'Cannot be less than sold quantity (${state.soldQty.toStringAsFixed(2)})'
                                      : null))
                          : null,
                    ),
                    onChanged: (value) {
                  ...
                  InkWell(
                    onTap: () async {
                      ...
                      if (picked != null && context.mounted) {
                        context.read<InvestmentFormBloc>().add(
                          InvestmentFormBuyDateChanged(picked),
                        );
                      }
                    },
                    ...
                    decoration: InputDecoration(
                      ...
                      errorText:
                          state.showValidation &&
                              (state.buyRate == null || state.buyRate! < 0)
                          ? 'Enter buy rate'
                          : null,
                    ),
                    onChanged: (value) {
                      final val = double.tryParse(value);
                      context.read<InvestmentFormBloc>().add(
                        InvestmentFormBuyRateChanged(val),
                      );
                    },
                  ),
                  const SizedBox(height: 16),
                  ...
                  AppSelectField<int?>(
                    ...
                    onChanged: (value) {
                      context.read<InvestmentFormBloc>().add(
                        InvestmentFormBrokerChanged(value),
                      );
                    },
                  ),
                  const SizedBox(height: 16),
                  ...
                  TextFormField(
                    ...
                    onChanged: (value) {
                      context.read<InvestmentFormBloc>().add(
                        InvestmentFormNotesChanged(value),
                      );
                    },
                  ),
                  ...
                  FilledButton(
                    onPressed: state.status == InvestmentFormStatus.submitting
                        ...
                        : () {
                            context.read<InvestmentFormBloc>().add(
                              const InvestmentFormBuySubmitted(),
                            );
                          },
                    child: Padding(
